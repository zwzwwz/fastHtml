...
class exportPSD {
	...
	openPSD() {
		...
		PSD.open(_self.file).then(function(psd) {
			...
			//将初始根节点配置写入treeJson.json
			//serverRes.end(JSON.stringify(tree.export(), undefined, 2));
			fs.writeFile("treeJson.txt", JSON.stringify(treeJson, undefined, 2), {
				encoding: "utf8"
			}, function(err) {
				//console.log(err);
			});

			//自定义根节点配置 设置node名称，保存图片---psd.js自带的接口
			...
			//将自定义根节点配置写入domJSON.json
			fs.writeFile("domJson.txt", JSON.stringify(domJSON, undefined, 2), {
				encoding: "utf8"
			}, function(err) {
				//console.log(err);
			});
	...
	createDivByJson(jsons) {
		...
		for (var i = 0; i < domJSON.children.length; i++) {
			...
			if (item.type == "layer" && item.visible && item.width && item.height) {
				...
				_self.cssStyle += '.page_' + _self.appName + ' .' + item.name + ' { position: absolute; top:50%; width:' + item.width /
					100 + 'rem; height:' + item.height / 100 + 'rem; left:' + item.left / 100 + 'rem; margin-top:' + -(_self.viewRect
						.height / 100 / 2 - item.top / 100) + 'rem; background:url(../' + (backGroundImgUrl + item.name) +
